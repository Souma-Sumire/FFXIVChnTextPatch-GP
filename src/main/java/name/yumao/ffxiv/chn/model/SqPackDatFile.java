	...
	public byte[] extractFile(long fileOffset, Boolean debugMode) throws IOException {
		...
		switch (contentType) {
			...
			case 3:
				...
				this.currentFilePointer.readShort(); // y1 - not used
				this.currentFilePointer.readShort(); // y2 - not used
				...
				for (i1 = 0; i1 < container.blockSizes.length; i1++) {
					...
					this.currentFilePointer.readInt(); // blockHeaderLength2 - not used
					this.currentFilePointer.readInt();
		...
		for (int j = 0; j < ((contentType == 4) ? blockCount : 1); j++) {
			...
			for (int i2 = 0; i2 < (dataBlocks[j]).length; i2++) {
				...
				this.currentFilePointer.readInt(); // blockHeaderLength - not used
				this.currentFilePointer.readInt();
				...
				if (debug)
					System.out.println("Decompressing block " + i2 + " @ file offset: " + this.currentFilePointer
							...
							String.format("%X", new Object[] { Integer.valueOf((dataBlocks[j][i2]).offset) })
							+ ". Compressed Size: " + compressedBlockSize + " and Decompressed Size: "
							+ decompressedBlockSize + ". Block Size: " + (dataBlocks[j][i2]).padding);
